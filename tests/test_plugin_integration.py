...
class TestPluginRegistration:
    ...
    def test_unquantized_linear_apply_fail_open_falls_back_to_reference(
        ...
        _reset_usage_state()
        monkeypatch.setenv("VLLM_NT_DEBUG_LINEAR_COMPARE", "1")
        monkeypatch.setenv("VLLM_NT_DEBUG_LINEAR_FAIL_OPEN_ATOL", "0.01")
        monkeypatch.setattr(
            "vllm_nt._ntops.patching.linear",
            lambda x, weight, bias=None: torch.zeros(
        ...
        torch.testing.assert_close(out, F.linear(x, DummyLayer.weight, bias))
