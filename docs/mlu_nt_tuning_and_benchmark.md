...
| 缺失模型 | `llama2_7b` 未在 `~/.cache/huggingface/hub` 找到 |
...
5. 将 `max_num_configs` 从 `1` 提升到 `10` 没有明显改善 MatMul 性能，说明当前瓶颈不只是调优搜索数量不足。
...
4. 如果补齐 `llama2_7b` 本地缓存，需要重新跑准确性、吞吐和 MatMul shape，避免报告只覆盖小模型。
